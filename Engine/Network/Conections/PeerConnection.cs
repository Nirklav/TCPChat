...
namespace Engine.Connections
{
  public class PeerConnection : IConnection, IDisposable
  {
    #region consts
    ...
    public void ConnectToService(IPEndPoint remotePoint, int serviceConnectId, ConnectionType type)
    {
      ...
      ClientModel.Client.Register(Id, FromServiceConnectId(serviceConnectId));
    ...
    public void SendMessage(ushort id, object messageContent) //TODO: убрать из метода выделение памяти
    {
      ThrowIfDisposed();
    ...
    public static string FromServiceConnectId(int serviceConnectId)
    {
      return string.Format("{0}{1}", ServiceConnectionPrefix, serviceConnectId);
    }
    #endregion
